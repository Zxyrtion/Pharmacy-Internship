...
class PurchaseOrder {
    ...
    public function createRequisition($pharmacist_id, $pharmacist_name, $department, $requisition_date, $date_required, $urgency, $reason, $items) {
        ...
        try {
            // Create requisition
            $req_sql = "INSERT INTO requisitions (requisition_id, pharmacist_id, pharmacist_name, requisition_date, urgency, status, total_amount, notes) 
                       VALUES (?, ?, ?, ?, ?, 'Submitted', ?, ?)";
            $stmt = $this->conn->prepare($req_sql);
            $stmt->bind_param("sisssds", $requisition_id, $pharmacist_id, $pharmacist_name, $requisition_date, $urgency, $total_amount, $reason);
            $stmt->execute();
