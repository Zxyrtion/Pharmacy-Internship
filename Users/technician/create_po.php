...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Add inventory report reference to comments for tracking
        $inventory_reference = "Inventory Report: $period by $reporter";
        if (!empty($comments)) {
            $comments .= " | $inventory_reference";
        } else {
            $comments = $inventory_reference;
        }
        ...
        if ($stmt->execute()) {
            ...
            $pharmacist_stmt = $conn->prepare("SELECT id FROM users WHERE role_name = 'Pharmacist'");
            if ($pharmacist_stmt) {
                $pharmacist_stmt->execute();
                $pharmacist_result = $pharmacist_stmt->get_result();
                while ($pharmacist = $pharmacist_result->fetch_assoc()) {
                    $message = "New Purchase Order #$po_number has been created by $full_name and requires your review and approval.";
                    createNotification($pharmacist['id'], $message, 'info', 'purchase_order', $po_id);
                }
...
if (isset($conn) && $period && $reporter) {
    ...
    if ($stmt) {
        ...
        while ($row = $res->fetch_assoc()) {
            $items_to_order[] = $row;
            $used_item_numbers[] = $row['item_number_name'];
        }
        $stmt->close();
    }
}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateTotals() {
                ...
                // Auto-calculate reasonable defaults if empty
                const tax = parseFloat(taxInput.value) || 0;
                const shipping = parseFloat(shippingInput.value) || 0;
                const other = parseFloat(otherInput.value) || 0;
                
                // Auto-calculate tax (12% of subtotal) if tax field is empty
                if (taxInput.value === '' || parseFloat(taxInput.value) === 0) {
                    const calculatedTax = subtotal * 0.12; // 12% VAT
                    taxInput.value = calculatedTax.toFixed(2);
                }
                
                // Auto-calculate shipping (₱150 or 5% of subtotal, whichever is higher) if shipping is empty
                ...
                // Auto-calculate other costs (₱50 flat fee) if other is empty
